...
float4 SampleEnv(LightLoopContext lightLoopContext, int index, float3 texCoord, float lod, float invExposureMultiplier, int sliceIdx = 0)
{
    // 31 bit index, 1 bit cache type
    ...
    if (lightLoopContext.sampleReflection == SINGLE_PASS_CONTEXT_SAMPLE_REFLECTION_PROBES)
    {
        ...
        color.rgb *= invExposureMultiplier;
    }
